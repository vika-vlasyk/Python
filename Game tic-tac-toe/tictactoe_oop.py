...
class TTTBoard:
    ...
    def isWinner(self, player):
        ...
        return (
            (s["1"] == s["2"] == s["3"] == p)
            or (s["4"] == s["5"] == s["6"] == p)  # Across the top
            or (s["7"] == s["8"] == s["9"] == p)  # Across the middle
            or (s["1"] == s["4"] == s["7"] == p)  # Across the bottom
            or (s["2"] == s["5"] == s["8"] == p)  # Down the left
            or (s["3"] == s["6"] == s["9"] == p)  # Down the middle
            or (s["3"] == s["5"] == s["7"] == p)  # Down the right
            or (s["1"] == s["5"] == s["9"] == p)  # Diagonal
        )  # Diagonal
